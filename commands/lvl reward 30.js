...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    if (timeout - (Date.now() - data.level30Timeout) > 0) {
      let timeLeft = ms(timeout - (Date.now() - data.level30Timeout));

      await interaction.reply({
        content: `you already claimed this reward come back when ur leveled up.`,
      });
    } else {
      data.level30Timeout = Date.now();
      data.wallet += amount * 40;
      await data.save();

      const weeklyEmbed = new discord.EmbedBuilder()
        .setColor("#F04a0c")
        .setDescription(
          `You recieved a level 30 reward`
        );
